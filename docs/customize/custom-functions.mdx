...
1. **No kwargs allowed in action definitions**: Actions functions must be defined to take positional args only (with type hints), or a single `params` arg containing a pydantic model if you need defaults and optionals.
2. **Return `ActionResult`**: All actions should return an `ActionResult | str | None`.
3. **Type hints are required**: They are used to verify that action params don't conflict with special arguments injected by the controller (e.g. `page`)
4. **Actions called directly must be passed kwargs***: When calling actions from other actions, you must **call using kwargs only**, even though the actions are defined using positional args only (for the same reasons as [pluggy](https://pluggy.readthedocs.io/en/stable/index.html#calling-hooks)).
