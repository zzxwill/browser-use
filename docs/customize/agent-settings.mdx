...
You can pass any of the following Playwright objects to the Agent: `page`, `browser_context`, `browser`, `browser_session`, or `browser_profile`. These all get passed down to create a `BrowserSession`:

```python
browser_session = browser_session or BrowserSession(page, browser_context, browser_profile)
```
