...
Hook functions are passed as parameters to `agent.run(on_step_start=..., on_step_end=...)`.  
...
- `agent.task` lets you see what the main task is, `agent.add_new_task(...)` lets you add a new one
- `agent.controller` give access to the `Controller()` object and `Registry()` of available actions
...
- `agent.sensitive_data` contains the sensitive data dict, which can be updated in place to add/remove items
...
- `agent.browser_session` gives direct access to the `BrowserSession()`, allowing use of all the browser APIs
    - `agent.browser_session.get_current_page()`: Get the current playwright `Page` object
