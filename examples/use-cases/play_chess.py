...
class PlayMoveParams(BaseModel):
    move: str = Field(description="The move in Standard Algebraic Notation (SAN) exactly as provided in the 'Legal Moves' list (e.g., 'Nf3', 'e4', 'Qh7#').")
...
def parse_transform(style: str) -> Optional[Tuple[float, float]]:
...
def create_fen_board(board_state: dict) -> str:
    ...
    for rank_num in RANKS:
        ...
        if rank_num != RANKS[-1]: fen += '/'
    return fen

async def get_current_board_info(page) -> Tuple[Optional[str], float]:
...
async def play_move(params: PlayMoveParams, browser: BrowserContext):
    ...
    try:
        ...
        start_click_x = start_x + click_offset; start_click_y = start_y + click_offset
        end_click_x = end_x + click_offset; end_click_y = end_y + click_offset
