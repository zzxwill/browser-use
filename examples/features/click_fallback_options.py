...
<head>
  ...
  <style>
    ...
    /* Display area showing the current selection */
    .select-display {
      padding: 10px;
...
<body>
  <div class="custom-select">
    ...
    <div class="select-options">
      ...
      <div class="select-option" data-value="option3">Pineaples</div>
    </div>
  </div>
...
<button onclick="alert('I told you!')">Don't click me</button>
  <script>
    ...
    // Toggle dropdown visibility when clicking on the display area
    selectDisplay.addEventListener('click', () => {
      const isVisible = selectOptions.style.display === 'block';
      selectOptions.style.display = isVisible ? 'none' : 'block';
    });

    // Handle option click: update the display and hide the dropdown
...
async def main():
    ...
    text_task = 'Open http://localhost:8000/, click element with the text "Select a fruit" and then click on Oranges'
    ...
    for task in [text_task, css_selector_task, xpath_task, text_task, select_task, button_task]:
        agent = Agent(
            task=task,
            llm=llm,
            controller=controller,
