...
#     docker run -v "$PWD/data":/data browseruse init
#     docker run -v "$PWD/data":/data browseruse add 'https://example.com'
#     docker run -v "$PWD/data":/data -it browseruse manage createsuperuser
#     docker run -v "$PWD/data":/data -p 8000:8000 browseruse server
# Multi-arch build:
#     docker buildx create --use
#     docker buildx build . --platform=linux/amd64,linux/arm64--push -t browseruse/browseruse:0.7.5 -t browseruse/browseruse:dev
#
# Read more about [developing browseruse](https://github.com/browseruse/browseruse#browseruse-development).
...
# Paths
ENV CODE_DIR=/app \
    CONFIG_DIR=/home/$BROWSERUSE_USER/.config/browseruse
...
RUN (echo "[i] Docker build for Browser Use $(cat /VERSION.txt) starting..." \
    ...
    && echo \
    && echo "CODE_DIR=${CODE_DIR} CONFIG_DIR=${CONFIG_DIR} PATH=${PATH}" \
    && echo \
    && uname -a \
...
RUN --mount=type=cache,target=/var/cache/apt,sharing=locked,id=apt-$TARGETARCH$TARGETVARIANT --mount=type=cache,target=/root/.cache/pip,sharing=locked,id=pip-$TARGETARCH$TARGETVARIANT --mount=type=cache,target=/root/.cache/ms-playwright,sharing=locked,id=browsers-$TARGETARCH$TARGETVARIANT \
    ...
    && chown -R "$DEFAULT_PUID:$DEFAULT_PGID" "/home/${BROWSERUSE_USER}/.config" \
    && ( \
