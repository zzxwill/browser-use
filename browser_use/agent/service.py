	...
	def get_trace_object(self) -> dict[str, Any]:
		...
		return {
			'trace': {
				...
				'git_info': json.dumps(get_git_info()) if get_git_info() else None,
				...
				'structured_output_truncated': (
					(json.dumps(structured_output.model_dump())[:20000] 
					 if len(json.dumps(structured_output.model_dump())) > 20000 
					 else json.dumps(structured_output.model_dump()))
					if structured_output else None
				),
				'action_history_truncated': (
					json.dumps(self.state.history.action_history())[:20000]
					if len(json.dumps(self.state.history.action_history())) > 20000
					else json.dumps(self.state.history.action_history())
				) if self.state.history.action_history() else None,
				'action_errors': json.dumps(self.state.history.errors()) if self.state.history.errors() else None,
				'urls': json.dumps(self.state.history.urls()) if self.state.history.urls() else None,
				...
				'usage': json.dumps(self.state.history.usage.model_dump()) if self.state.history.usage else None,
			},
			'trace_details': {
				...
				# AgentHistoryList methods
				'structured_output': json.dumps(structured_output.model_dump()) if structured_output else None,
				'final_result_response': final_result,
				'complete_history': json.dumps(self.state.history.model_dump()),
				'action_history': json.dumps(self.state.history.action_history()) if self.state.history.action_history() else None,
			}
		}
