...
def cleanup_html(html_content):
	...
	def is_visible(element):
		...
		return not (
			...
			or element.get('hidden')
			or element.get('style')
			and (
				'display: none' in element.get('style')
				or 'visibility: hidden' in element.get('style')
			)
			or element.get('class')
			and (
				'hidden' in ' '.join(element.get('class'))
				or 'invisible' in ' '.join(element.get('class'))
			)
		)
	...
	def is_leaf_element(element):
		...
		return (
			bool(element.string and element.string.strip())
			and not element.find_all(True)
			and element.name not in ['script', 'style', 'meta', 'link', 'svg']
		)
	...
	return {'html': cleaned_html, 'main_content': main_content}
