    ...
    def click_element_by_index(self, index: int):
        ...
        try:
            ...
            for strategy in [
                ...
                lambda: self.wait.until(EC.element_to_be_clickable(
                    (By.XPATH, f"//*[@id='{xpath.split('id=')[-1].split(']')[0]}']" if 'id=' in xpath else xpath)
                )),
                # Try finding by any visible matching element
                lambda: next(elem for elem in self.driver.find_elements(By.XPATH, "//*[not(ancestor-or-self::*[@style='display: none'])]")
